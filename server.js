...
const MAX_HISTORY = 200; // keep last 200 recordings - Main, Group and Direct
...
try {
    history = JSON.parse(fs.readFileSync(META, 'utf8'));
} catch (e) {}

const saveMeta = () => {
    fs.writeFile(META, JSON.stringify(history), () => {});
};
...
const saveDirectory = () => {
    fs.writeFile(DIR_FILE, JSON.stringify(directory), () => {});
};
...
const onlineCount = () =>
    [...wss.clients].filter((c) => c.user).length;


function persistId(u) {

    return u.role === 'lecturer'
        ? 'staff:' + u.staffId.toLowerCase()
        : 'stu:' + u.uucms.toLowerCase();
}
...
function myGroups(pid) {
    ...
    return Object.values(groups)
        .filter((g) => g.members.includes(pid));
}
...
function endSpeaker(key) {
    ...
    // Tell the correct people that speaker stopped
    targetRecipients(
        lock.target,
        lock.ws
    ).forEach((c) => {
    ...
    ws.on('message', (data, isBinary) => {
        ...
        if (msg.type === 'login') {
            ...
            if (msg.role === 'lecturer') {
                ...
                if (msg.pass !== STAFF_PASS) {

                    return sendJSON(
                        ws,
                        {
                            type: 'error',
                            msg: 'Wrong lecturer password'
                        }
                    );
                }
                ...
                user = {
                    ...
                    uucms: clean(msg.uucms),

                    uClass: clean(msg.uClass)
                };
